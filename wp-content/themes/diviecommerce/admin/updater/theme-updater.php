...
// Config settings
    array(
        ...
        'download_id'            => 297076, // Optional, used for generating a license renewal link
    ...
    // Strings
    array(
        'theme-license'                        => __('Theme License', 'divi_ecommerce'),
        'enter-key'                            => __('Enter your theme license key.', 'divi_ecommerce'),
        'license-key'                          => __('License Key', 'divi_ecommerce'),
        'license-action'                       => __('License Action', 'divi_ecommerce'),
        'deactivate-license'                   => __('Deactivate License', 'divi_ecommerce'),
        'activate-license'                     => __('Activate License', 'divi_ecommerce'),
        'status-unknown'                       => __('License status is unknown.', 'divi_ecommerce'),
        'renew'                                => __('Renew?', 'divi_ecommerce'),
        'unlimited'                            => __('unlimited', 'divi_ecommerce'),
        'license-key-is-active'                => __('License key is active.', 'divi_ecommerce'),
        'expires%s'                            => __('Expires %s.', 'divi_ecommerce'),
        'expires-never'                        => __('Lifetime License.', 'divi_ecommerce'),
        '%1$s/%2$-sites'                       => __('You have %1$s / %2$s sites activated.', 'divi_ecommerce'),
        'license-key-expired-%s'               => __('License key expired %s.', 'divi_ecommerce'),
        'license-key-expired'                  => __('License key has expired.', 'divi_ecommerce'),
        'license-keys-do-not-match'            => __('License keys do not match.', 'divi_ecommerce'),
        'license-is-inactive'                  => __('License is inactive.', 'divi_ecommerce'),
        'license-key-is-disabled'              => __('License key is disabled.', 'divi_ecommerce'),
        'site-is-inactive'                     => __('Site is inactive.', 'divi_ecommerce'),
        'license-status-unknown'               => __('License status is unknown.', 'divi_ecommerce'),
        'update-notice'                        => __("Updating this theme will lose any customizations you have made. 'Cancel' to stop, 'OK' to update.", 'divi_ecommerce'),
        'update-available'                     => __('%1$s is available. %2$sCheck out what\'s new%3$s or %4$supdate now%5$s.', 'divi_ecommerce'),
        'update-available-expired-license-key' => __('%1$s is available. %2$sRenew your license key%3$s to update.', 'divi_ecommerce')
    )
);

// Load translations
load_theme_textdomain('divi_ecommerce', dirname(__FILE__) . '/lang');
...
class AGS_THEME_Updater_Admin {
            ...
            <form method="post" action="options.php" id="AGS_THEME_license_form">
                <div id="AGS_THEME_license_form_header">
                    <a href="https://wpzone.co/" target="_blank">
                        <img src="<?php echo(get_theme_root_uri() . '/' . $this->theme_slug . '/admin/updater/wp_zone_logo.png'); ?>" alt="WP Zone"/>
                    </a>
                </div>
                ...
                <div id="AGS_THEME_license_form_body">
                    ...
                    <p>
                        <?php
                        printf(
                            __('Thank you for purchasing %s. <br />Please enter your license key below.', 'divi_ecommerce'),
                            $this->item_name);
                        ?>
                    </p>
                    ...
                    <label>
                        <span><?php _e('License Key:', 'divi_ecommerce'); ?></span>
                    ...
                    if (isset($_GET['sl_theme_activation']) && $_GET['sl_theme_activation'] == 'false') {
                        echo('<p id="AGS_THEME_license_form_error">' . (empty($_GET['sl_message']) ? esc_html__('An unknown error has occurred. Please try again.', 'divi_ecommerce') : esc_html($_GET['sl_message'])) . '</p>');
                    }
                    submit_button(__('Continue', 'divi_ecommerce'));
                    ?>
                </div>
            </form>
        </div>
    ...
    function get_api_response($api_params) {
        ...
        if (is_wp_error($response)) {
            wp_die($response->get_error_message(), __('Error', 'divi_ecommerce') . $response->get_error_code());
        }
    ...
    function activate_license() {
        ...
        if (is_wp_error($response) || 200 !== wp_remote_retrieve_response_code($response)) {
            ...
            if (is_wp_error($response)) {
                ...
                $message = __('An error occurred, please try again.', 'divi_ecommerce');
            }
            ...
            if (false === $license_data->success) {
                ...
                switch ($license_data->error) {
                    ...
                    case 'expired' :
                        ...
                        $message = sprintf(
                            __('Your license key expired on %1$s. Please visit our website to renew.', 'divi_ecommerce'),
                    ...
                    case 'revoked' :
                        ...
                        $message = __('Your license key has been disabled.', 'divi_ecommerce');
                        break;

                    case 'missing' :

                        $message = __('Invalid license key.', 'divi_ecommerce');
                        break;

                    case 'invalid' :
                    case 'site_inactive' :

                        $message = __('Your license is not active for this URL.', 'divi_ecommerce');
                        break;

                    case 'item_name_mismatch' :

                        $message = sprintf(__('This appears to be an invalid license key for %s.', 'divi_ecommerce'), $args['name']);
                        break;

                    case 'no_activations_left':

                        $message = __('Your license key has reached its activation limit. Please deactivate the key on one of your other sites before activating it on this site.', 'divi_ecommerce');
                        break;
    ...
    function deactivate_license() {
        ...
        if (is_wp_error($response) || 200 !== wp_remote_retrieve_response_code($response)) {
            ...
            if ($license_data && ($license_data->license == 'deactivated')) {
                ...
                $message = __('An error occurred while deactivating your license key.', 'divi_ecommerce');
            }
        ...
        <form method="post" id="AGS_THEME_license_form">
            ...
            <div id="AGS_THEME_license_form_body">
                ...
                <p class="description">
                    ...
                    if ($licenseKeyExpiry == 'lifetime') {
                        esc_html_e('This license key does not expire.', 'divi_ecommerce');
                    } else if ($isExpired) {
                        printf(esc_html__('Expired on %1$s. %2$sRenew now%3$s to continue receiving updates and support.', 'divi_ecommerce'),
                        ...
                        printf(esc_html__('Expires on %1$s. %2$sClick here%3$s to renew.', 'divi_ecommerce'),
                            date_i18n(get_option('date_format'), $licenseKeyExpiry + (get_option('gmt_offset') * 3600)),
                            '<a href="' . esc_attr($this->get_renewal_link()) . '" target="_blank">',
                            '</a>');
                    }
                ...
                submit_button(__('Deactivate License Key', 'divi_ecommerce'), '');
                ?>
            </div>
        </form>
